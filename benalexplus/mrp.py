...
class MRP_INVOICE:
    ...
    TAX_BASE_FOREIGN = 'ZAKL0'
    TAX_BASE_DOMESTIC = 'ZAKL2'
    TOTAL = 'CELKEM'
...
class MrpService:
    ...
    def _get_invoices_base(self, where_clause=None, having_clause=None):
        ...
        query = f'''
            SELECT
                ...
                {MRP_TABLE.INVOICE}.{MRP_INVOICE.DUE_DATE} AS DUE_DATE,
                {MRP_TABLE.INVOICE}.{MRP_INVOICE.TAX_BASE_FOREIGN} AS TAX_BASE_FOREIGN,
                {MRP_TABLE.INVOICE}.{MRP_INVOICE.TAX_BASE_DOMESTIC} AS TAX_BASE_DOMESTIC,
            ...
            GROUP BY
                ...
                {MRP_TABLE.INVOICE}.{MRP_INVOICE.DUE_DATE},
                {MRP_TABLE.INVOICE}.{MRP_INVOICE.TAX_BASE_FOREIGN},
                {MRP_TABLE.INVOICE}.{MRP_INVOICE.TAX_BASE_DOMESTIC},
                {MRP_TABLE.INVOICE}.{MRP_INVOICE.TOTAL},
    ...
    def get_products_states(self, mrp_products_ids=None, hash_states=True):
        WHERE = f'''
        ...
        query = f'''
            SELECT
                ...
                {MRP_TABLE.PRODUCT}.{MRP_PRODUCT.ESHOP_FLAG},
                {MRP_TABLE.PRODUCT}.{MRP_PRODUCT.WARRANTY},
                CAST(SUM({MRP_TABLE.PRODUCT_STATUS}.{MRP_PRODUCT_STATUS.STOCK_QUANTITY}) AS INTEGER),
                {MRP_TABLE.PRODUCT}.{MRP_PRODUCT.STOCK_MINIMUM},
        ...
        started = perf_counter()
        results = [(ps[0], get_hash(ps[1:])) for ps in products_states] if hash_states else products_states  # tuple(id, hash)
        logger.debug('Hashed in %fs', (perf_counter() - started))
        return results
    ...
    def set_product_category(self, mrp_product_id, mrp_category_number):
        query = f'''
            UPDATE {MRP_TABLE.PRODUCT} SET {MRP_PRODUCT.CATEGORY_NUMBER} = {mrp_category_number} WHERE {MRP_PRODUCT.ID} = {mrp_product_id}
        '''
        self._execute(query)
    ...
    def sync_product_category_ex(self, mrp_product_id, mrp_product_number, mrp_category_numbers):
        self._execute(f'''
            DELETE FROM {MRP_TABLE.PRODUCT_CATEGORY_EX} WHERE {MRP_PRODUCT_CATEGORY_EX.PRODUCT_ID} = {mrp_product_id}
        ''')
        for mrp_category_number in mrp_category_numbers:
            query = f'''
                INSERT INTO {MRP_TABLE.PRODUCT_CATEGORY_EX} (
                    {MRP_PRODUCT_CATEGORY_EX.PRODUCT_ID},
                    {MRP_PRODUCT_CATEGORY_EX.PRODUCT_NUMBER},
                    {MRP_PRODUCT_CATEGORY_EX.CATEGORY_NUMBER}
                ) VALUES (
                    {mrp_product_id},
                    {mrp_product_number},
                    {mrp_category_number}
